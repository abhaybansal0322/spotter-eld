...
- `total_miles_driving` is the sum of each driving event's mile span, from its own mile to the next event's, not driving minutes at `AVG_SPEED_MPH`. Driving time to a stop rounds up to the 15-minute grid, so minutes times speed overstated distance (440 mi for a 422-mile Amarillo to Denver day); positions do not, because the engine sets the mile to the stop's true mile on arrival. A trip's sheets therefore add up to its route distance. A day that ends mid-leg carries the estimated position, and the next day absorbs the rounding, so a day's implied speed can dip below 55 mph but its miles are never negative: the last stretch of a leg is at least 15 minutes, more than the at-most-14-minute rounding.
- Total mileage today equals total miles driving today. On the form the first is the vehicle's miles and the second the driver's; they differ only with a co-driver or another driver in the same truck, and every plan here is solo.
